...
class Pyarchinit_pyqgis(QDialog, Settings):
	...
	def charge_vector_layers_ipogeo(self, id_ipogeo):
		self.id_ipogeo = id_ipogeo
		#Clean Qgis Map Later Registry
		#QgsMapLayerRegistry.instance().removeAllMapLayers()
		# Get the user input, starting with the table name
		...
		if settings.SERVER == 'sqlite':
			...
			id_ipogeo_string =  ('%s%d%s') % ("id_ipogeo = ",self.id_ipogeo,"'")


			srs = QgsCoordinateReferenceSystem(self.SRS, QgsCoordinateReferenceSystem.PostgisCrsId)

			uri.setDataSource("public", "pyarchinit_ipogei_view", "the_geom", id_ipogeo_string, "gid")
			layerIpogeo = QgsVectorLayer(uri.uri(), "Ipogei", "postgres")

			if  layerIpogeo.isValid() == True:
				layerIpogeo.setCrs(srs)
				self.IpogeoLayerId = layerIpogeo.getLayerID()
				#style_path = ('%s%s') % (self.LAYER_STYLE_PATH, 'ipogei.qml')
				#layerIpogeo.loadNamedStyle(style_path)
				QgsMapLayerRegistry.instance().addMapLayer(layerIpogeo, True)
